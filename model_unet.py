...
class ReconstructiveSubNetwork(nn.Module):
    def __init__(self,in_channels=3, out_channels=3, base_width=128):
        ...
        self.encoder = EncoderReconstructive(in_channels, base_width) # 重构编码器
        self.decoder = DecoderReconstructive(base_width, out_channels=out_channels) # 重构解码器
...
class EncoderReconstructive(nn.Module):
    def __init__(self, in_channels, base_width):
        ...
        self.block1 = nn.Sequential(
            nn.Conv2d(in_channels,base_width, kernel_size=3, padding=1), # 宽高输出保持不变
            nn.BatchNorm2d(base_width),
            nn.ReLU(inplace=True),
            nn.Conv2d(base_width, base_width, kernel_size=3, padding=1), # 宽高输出保持不变
            nn.BatchNorm2d(base_width),
            nn.ReLU(inplace=True))
        self.mp1 = nn.Sequential(nn.MaxPool2d(2))
        self.block2 = nn.Sequential(
...
class DecoderReconstructive(nn.Module):
    def __init__(self, base_width, out_channels=1):
        ...
        self.up1 = nn.Sequential(nn.Upsample(scale_factor=2, mode='bilinear', align_corners=True), # 宽高是原来的2倍
                                 nn.Conv2d(base_width * 8, base_width * 8, kernel_size=3, padding=1),
                                 nn.BatchNorm2d(base_width * 8),
                                 nn.ReLU(inplace=True))
        self.db1 = nn.Sequential(
